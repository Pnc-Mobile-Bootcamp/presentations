...
// fun String?.orUnavailable(): String = if (this.isNullOrBlank()) "Unavailable" else this

// val description: String? = null
// println(description.orUnavailable())

// val name: String? = "Mountain Bike"
// println(name.orUnavailable())
...
//  1. add an empty screen state
//  2. write a function with an exhaustive "when" that produces a message for every state
//  3. add an extension method to ProductSummary, called salePrice(percent: Double): Double
//  4. create two instances of ProductSummary and use copy() to make a revised version of one
//  5. print out both instance to demonstrate that the original is unchanged
...
fun screenMessage(state: ProductScreenState): String =
    when (state) {
        ...
        is ProductScreenState.Success -> "Showing ${state.products.size} products"
        is ProductScreenState.Error -> "Unable to load products: ${state.message}"
        is ProductScreenState.Empty -> "No available products"
    }

println(screenMessage(ProductScreenState.Loading))
println(screenMessage(ProductScreenState.Error("Missing credentials")))
println(screenMessage(ProductScreenState.Empty))


fun ProductSummary.salePrice(percent: Double): Double = price * (1 - percent / 100)

val p1 = ProductSummary(101, "Mountain Bike", 1000.0)
val p2 = p1.copy(price = p1.salePrice(10.0))
